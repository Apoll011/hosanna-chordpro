...
describe("ChordPro formatter", () => {
  it("parses and rebuilds directive aliases", () => {
    ...
    assert.equal(
      buildDirectiveLine(parsed!, { expandDirectiveAliases: true }),
      "{title: Song}",
    );
  ...
  it("normalizes chord notation and validates chord tokens", () => {
    ...
    assert.equal(
      normalizeChordContent(" c / e ", { normalizeNotationAliases: true })
        .result,
      "C/E",
    );
  });
  ...
  it("formats aliases, whitespace, and malformed directives", () => {
    ...
    assert.equal(
      detectMalformedDirective("{title: Song", 4)?.type,
      "malformed_directive",
    );
  });
});
